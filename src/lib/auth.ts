...
export function getAuth(env: { DB_ASTRO_TUTORIAL: D1Database; BETTER_AUTH_SECRET?: string; BETTER_AUTH_URL?: string }, request?: Request) {
    const db = new Kysely({
    ...
    return betterAuth({
        ...
        baseURL: env.BETTER_AUTH_URL || (request ? new URL(request.url).origin : "http://localhost:4321"),
        emailAndPassword: {
            enabled: true,
        },
